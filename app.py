...
def classify_mutation(wt_protein: str, mut_protein: str) -> dict:
    if wt_protein == mut_protein:
        return {"type": "silent", "position": None, "aa_from": None, "aa_to": None}

    # kısalmışsa nonsense (erken stop)
    if len(mut_protein) < len(wt_protein):
        ...
        for i, (a, b) in enumerate(zip(wt_protein, mut_protein), start=1):
            if a != b:
                pos = i
                aa_from = a
                aa_to = b if i <= len(mut_protein) else None
                break
        if pos is None:
            pos = len(mut_protein) + 1
            aa_from = wt_protein[pos - 1]
            aa_to = None
        return {"type": "nonsense", "position": pos, "aa_from": aa_from, "aa_to": aa_to}

    # uzunluk aynıysa missense varsay
    pos = None
    aa_from = aa_to = None
    for i, (a, b) in enumerate(zip(wt_protein, mut_protein), start=1):
        if a != b:
            ...
            aa_from, aa_to = a, b
            break
    return {"type": "missense", "position": pos, "aa_from": aa_from, "aa_to": aa_to}
